...
class _ControlUsersPage extends State<ControlUsersPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        centerTitle: true,
        title: Text(
          !selectMode ? "Управление пользователями" : "Выберите пользователя",
          style: TextStyle(
  ...
  Widget _userGrid(UserEntity user) {
    debugPrint("Load user ${user.name} id: ${user.id} ${user.phone}");
    return InkWell(
      onTap: selectMode
          ? () {
              ...
              child: Column(
                children: [
                  ...
                  Container(
                    decoration: BoxDecoration(
                        border:
                            Border.all(color: Color(0xFFAEB4B7), width: 0.3.w),
                        borderRadius: BorderRadius.circular(15)),
                    child: Padding(
  ...
  Widget _userList(UserEntity user) {
    if (user.photo != null) {
      debugPrint(
          "Load user ${user.name} id: ${user.id} ${user.phone} photo:${user.photo}");
      if (user.photo!.split(".")[1] == "heic") {
        debugPrint(
            "Load user ${user.name} id: ${user.id} ${user.phone} photo:${user.photo}");
        user.photo = null;
      }
                ...
                Row(
                  ...
                  children: [
                    Container(
                      decoration: BoxDecoration(
                          border: Border.all(
                              color: Color(0xFFAEB4B7), width: 0.3.w),
                          borderRadius: BorderRadius.circular(15)),
                      child: Padding(
                        padding: EdgeInsets.all(2.w),
                        child: Text(
