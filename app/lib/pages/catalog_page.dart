...
class CatalogPage extends StatefulWidget {
  ...
  @override
  State<StatefulWidget> createState() => CatalogPageState();
}
...
class CatalogPageState extends State<CatalogPage> {
        ...
        child: FutureBuilder(
          ...
          builder: (context, snapshot) {
            if (snapshot.hasData) {
              ...
              debugPrint("Company error ${snapshot.error}");
              return Center(
  ...
  Widget _card(CompanyEntity companyEntity) {
    return Container(
      ...
      decoration: BoxDecoration(borderRadius: BorderRadius.circular(15), color: const Color(0xffEDEEEF)),
      child: Padding(
        ...
        child: Column(
          ...
          children: [
                ...
                ? SizedBox(
                    child: SizedBox(
                      ...
                      child: ClipRRect(
                        ...
                        child: Image.network(
                          GlobalsWidgets.getPhoto(companyEntity.image),
                          fit: BoxFit.cover,
                        ),
                      ),
                    ),
            ...
            (companyEntity.manager != null && companyEntity.manager!.uid != GlobalsWidgets.uid)
                ? SizedBox(
                    height: 1.h,
                  )
                : const SizedBox.shrink(),
                ...
                ? Align(
                    ...
                    child: SizedBox(
                      child: OutlinedButton(
                          style: OutlinedButton.styleFrom(backgroundColor: Colors.transparent, side: BorderSide(color: GlobalsColor.blue), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(9), side: BorderSide.none)),
                          onPressed: () {
                            Dio dio = Dio();
                            RestClient client = RestClient(dio);
                            client.findChat(GlobalsWidgets.uid, companyEntity.manager!.uid, null).then((value) {
                              Navigator.push(context, MaterialPageRoute(builder: (context) => CustomChatPage(showTitle: true, title: companyEntity.name, chatName: value)));
                            });
                          },
                          child: Text(
