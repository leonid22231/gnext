...
class TransportationPage extends StatefulWidget {
  ...
  const TransportationPage({required this.sub, required this.city, super.key});
...
class TransportationPageState extends State<TransportationPage>
                  ...
                  (kuda != null && otkuda != null)
                      ? FutureBuilder(
                          future: RestClient(Dio()).searchOrders(
                              OrderMode.NEW, kuda!.id, otkuda!.id),
                          builder: (context, snapshot) {
                            if (snapshot.hasData) {
  ...
  Future<List<OrderEntity>> getMyOrders(bool out) {
    ...
    return client.myOrders(GlobalsWidgets.uid, out, OrderMode.CLASSIC);
  }
}
