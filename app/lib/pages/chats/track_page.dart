...
class _TrackPageState extends State<TrackPage>{
  ...
  String chat_name = GlobalsWidgets.chats[0];
  ...
  Widget build(BuildContext context) {
    return CustomChatPage(title: S.of(context).page2, chatName: chat_name, showTitle: false,);
  }
  Widget _storis(){
    return FutureBuilder(future: getStories(), builder: (context, snapshot){
      ...
      if(snapshot.hasData){
        ...
        widgets.addAll(list.map((e) => user(e.user!.name, false, true, e.index!)).toList());
        return SizedBox(
          height: 12.h,
  ...
  Widget user(String name, bool active,bool current, int index){
    return InkWell(
      onTap: !current?() async {
        showDialog(context: context, builder: (context){
          return AlertDialog(
